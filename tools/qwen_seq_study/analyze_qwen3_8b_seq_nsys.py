...
def parse_args():
    ...
    parser.add_argument(
        "--input-root",
        type=Path,
        default=Path("runs/qwen8b_seq_study_tp4_nsys"),
        help="Root directory containing prof_seq*/seq*.nsys-rep outputs.",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("runs/qwen8b_seq_study_tp4_nsys/analysis"),
    ...
    parser.add_argument(
        ...
        type=int,
        nargs="+",
        default=[512, 1024, 2048, 3072, 4096, 5120, 6144, 7168, 8192, 9216, 10240, 11264, 12288, 13312, 14336, 15360],
        help="Sequence lengths to aggregate in order.",
    )
    parser.add_argument(
        "--label",
        default="Qwen3-8B TP4",
...
def main():
    ...
    save_stacked_bar_chart(
        ...
        title=f"{args.label} Transformer Forward+Backward GPU Breakdown",
        total_key="transformer_block_total_time_ms",
        part_keys=("attention_total_time_ms", "ffn_total_time_ms", "transformer_others_total_time_ms"),
        legend_labels=("Attention", "FFN", "Transformer Others"),
    )
    save_ratio_line_chart(
        successful_rows,
        args.output_root / "qwen3_8b_seq_total_ratios.png",
        title=f"{args.label} Transformer Forward+Backward GPU Ratios",
        ratio_keys=("attention_total_ratio", "ffn_total_ratio", "transformer_others_total_ratio"),
        legend_labels=("Attention", "FFN", "Transformer Others"),
    )
    save_stacked_bar_chart(
        successful_rows,
    ...
    save_step_time_chart(
        ...
        title=f"{args.label} Step GPU Time",
    )
